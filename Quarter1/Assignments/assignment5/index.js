...
const addInArray = (array1, index, insert) => {
...
const cartA = (quantity, inputArrA, index1, insert1) => {
...
const afterUpdateQuantity = [];
const cartC = (updateQuantity) => {
    let q1 = 0;
...
const sumArrayResult = sumArray([2, 3, 4, 5, 6, 7, 8]);
...
const celsiusToFahrenheit = (arrayTemp) => {
    const newArray = [];
    let i = 0;
    while (i < arrayTemp.length) {
        const temp = (9 / 5 * arrayTemp[i]) + 32;
        newArray.push(temp);
        i++;
    }
...
const arr1 = [1, 3, -2, -9, -5, 4, 6];
const afterRemoving = arr1.filter((val) => val >= 0);
...
const multipliedBy2 = arr2.map((val) => val * 2);
...
const arr3 = ["apple", "banana", "cherry", "date", "grape"];
const filtering = arr3.filter((val) => val.length > 5);
...
const filteringA = arr4.filter((val) => val % 2 === 0);
const squares = filteringA.map((val) => val * val);
...
const arr5 = [0, 10, 20, 30, 40];
const fahrenheit = arr5.map((val) => (val * 9 / 5) + 32);
...
const arr6 = [3, 6, 9, 12, 15, 18];
const doubledVal = arr6.filter((val) => val % 2 !== 0).map((val) => val * 2);
...
const arr7 = ["Alice", "Bob", "Charlie", "David", "Emily"];
arr7.forEach((val) => console.log(val + '!'));
